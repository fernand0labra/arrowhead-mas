...
public class OrchestratorDriver {
	...
	//-------------------------------------------------------------------------------------------------
	public AnalysisResponseDTO analyseServiceRequest(final AnalysisRequestDTO request) {
		logger.debug("analyseServiceRequest started...");
		Assert.notNull(request, "AnalysisRequestDTO is null.");
		
		// TODO final UriComponents analysisUri = getMismatchAnalysisUri();
		final UriComponents analysisUri = UriComponentsBuilder.newInstance()
			      .scheme("http").host("127.0.0.1").port(8999).path("/analysis").build();
		
		final ResponseEntity<AnalysisResponseDTO> response = httpService.sendRequest(analysisUri, HttpMethod.POST, AnalysisResponseDTO.class, request);
	...
	private UriComponents getMismatchAnalysisUri() {
		logger.debug("getMismatchAnalysisUri started...");
		
		if (arrowheadContext.containsKey(CoreCommonConstants.MISMATCH_ANALYSIS_URI)) {
			try {
				return (UriComponents) arrowheadContext.get(CoreCommonConstants.MISMATCH_ANALYSIS_URI);
			} catch (final ClassCastException ex) {
				throw new ArrowheadException("Orchestrator can't find Mismatch Analysis URI.");
			}
		}
