...
public class OrchestratorService {
	...
	public OrchestrationResponseDTO dynamicOrchestration(final OrchestrationFormRequestDTO request) {
		...
		// If the mismatch analysis is requested we call the mismatchAnalysis System 
		if(flags.get(Flag.ENABLE_MISMATCH_ANALYSIS)) {
			String serviceDefinition = request.getRequestedService().getServiceDefinitionRequirement();
			...
			consumer.add(request.getRequesterSystem().getSystemName());
						
			ArrayList<String> providers = new ArrayList<String>();
			for(OrchestrationResultDTO dto : orList)
				providers.add(dto.getProvider().getSystemName());
			
			systems.put("consumer", consumer);
			systems.put("providers", providers);
			
			AnalysisRequestDTO analysisRequestDTO = new AnalysisRequestDTO(serviceDefinition, systems);
			AnalysisResponseDTO analysisResponseDTO = orchestratorDriver.analyseServiceRequest(analysisRequestDTO);
						
			System.out.println(analysisResponseDTO.toString());
			analysisResponseDTO.summarize();
			
			// Call the subsequent solver system ...
			if(analysisResponseDTO.getFlag().equals("NOT_OK"))
				orList.clear();
			else {
				OrchestrationResultDTO selectedSystem = null;
				
				// Select the orchestration result that matches the selected system
				for(OrchestrationResultDTO dto : orList)
					if(dto.getProvider().getSystemName().equals(analysisResponseDTO.getSystem()))
						selectedSystem = dto;
				...
				if(analysisResponseDTO.getFlag().equals("ALTER_T"))
					System.out.print(""); // TODO Call Translator System
				else if(analysisResponseDTO.getFlag().equals("ALTER_G"))
					System.out.print(""); // TODO Call Interface Generator System
			}
		}
