...
public class ProviderApplicationInitListener extends ApplicationInitListener {
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		final ServiceRegistryRequestDTO analyseServiceRequest = createServiceRegistryRequest(AnalysisProviderConstants.ANALYSIS_SERVICE_DEFINITION, AnalysisProviderConstants.ANALYSIS_URI, HttpMethod.POST);
		analyseServiceRequest.getMetadata().put(AnalysisProviderConstants.REQUEST_PARAM_KEY_SERVICE_DEFINITION, AnalysisProviderConstants.REQUEST_PARAM_SERVICE_DEFINITION);
		analyseServiceRequest.getMetadata().put(AnalysisProviderConstants.REQUEST_PARAM_KEY_SYSTEMS, AnalysisProviderConstants.REQUEST_PARAM_SYSTEMS);
		arrowheadService.forceRegisterServiceToServiceRegistry(analyseServiceRequest);
	}
	...
	//-------------------------------------------------------------------------------------------------
		private ServiceRegistryRequestDTO createServiceRegistryRequest(final String serviceDefinition, final String serviceUri, final HttpMethod httpMethod) {
			...
			if (sslEnabled && tokenSecurityFilterEnabled) {
				...
				serviceRegistryRequest.setInterfaces(List.of(AnalysisProviderConstants.INTERFACE_SECURE));
			} else if (sslEnabled) {
				systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
				...
				serviceRegistryRequest.setInterfaces(List.of(AnalysisProviderConstants.INTERFACE_INSECURE));
			}
			serviceRegistryRequest.setProviderSystem(systemRequest);
			serviceRegistryRequest.setServiceUri(serviceUri);
			serviceRegistryRequest.setMetadata(new HashMap<>());
			serviceRegistryRequest.getMetadata().put(AnalysisProviderConstants.HTTP_METHOD, httpMethod.name());
